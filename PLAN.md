...
Adapts the brief's proposed shape to house conventions (flat package layout per `dol`, not
`src/`; `py/` + `ts/` to host both languages in one repo).

```
walkthru/
  README.md  PLAN.md  DECISIONS.md  LICENSE  .gitignore
  misc/docs/                     # the brief + the two reports (source of truth for intent)
  py/                            # PyPI "walkthru"
    pyproject.toml               # hatchling, flat layout, py>=3.10, MIT, wads uv-CI, ruff D100
    walkthru/
      __init__.py
    ...
    tests/
  ts/                            # npm "acture-walkthru"
    package.json
    src/
      core/                      # PURE: play(), lifecycle, CommandSource, codegened Zod SSOT
      ports/                     # port interfaces
      adapters/                  # playwright/, driverjs/, motion/, webspeech/, kokoro/
      ecosystem/                 # acture/ (registry tap + replay), zodal/, reelee/
    tests/
  schema/                        # JSON Schema (emitted) + codegened Zod + round-trip fixtures
