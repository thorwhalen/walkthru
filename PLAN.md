...
walkthru/                        # repo root
  ...
  walkthru/                      # PyPI "walkthru" — package at root (wads name/name form)
    ...
    observers/                   # ready-made observers (wall-clock pacing)
  tests/                         # Python tests (schema, engine, timeline, firewall, adapters)
  ts/                            # npm "acture-walkthru" — single TS package
    package.json                 #   incl. the `wads.ci` block (npm CI/publish SSOT)
    src/                         #   index.ts + schema.generated.ts + *.test.ts (vitest, colocated)
    scripts/codegen.mjs          #   JSON Schema → Zod codegen (+ --check drift guard)
  schema/                        # JSON Schema (emitted from Pydantic) + fixtures
  examples/                      # generative + capture + narration demos
  misc/docs/                     # the brief + the two reports (intent SSOT)
  .github/workflows/
    ci.yml                       # Python → i2mint/wads reusable uv-ci.yml (paths-ignore: ts/**)
    npm-ci-ts.yml                # TypeScript → i2mint/wads reusable npm-ci.yml (paths: ts/**)
```

> **Frontend CI landed on the wads `ts` overlay.** It was deferred at first, pending
> **i2mint/wads#39** (generalize the frontend overlay into a js/ts language-profile registry), so
