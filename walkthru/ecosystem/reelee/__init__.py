...
:func:`render_plans` are the render drivers (with injectable ``film_renderer`` for testing), and
:func:`camera_path_builder` makes the camera follow the document's camera track.
:func:`to_production_manifest` turns a rendered film into a commentary production braidio can
import, so a tour opens (and edits) in reelee's studio like any other film. See
